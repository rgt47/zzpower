...
create_generic_test_server <- function(id, test_spec,
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    output$download_methods_md <- shiny::downloadHandler(
      filename = function() {
        sprintf("zzpower_methods_%s_%s.md",
                test_spec$id,
      ...
      content = function(file) {
        ...
        writeLines(c(
          sprintf("# Methods paragraph: %s", test_spec$name),
          sprintf("*Generated by zzpower on %s.*",
    ...
    sensitivity_table_with_power <- shiny::reactive({
      ...
      ss <- test_spec$sample_size_calc(.collect_params())

      df$power_at_n <- vapply(df$effect_size_std, function(es) {
        .compute_power(test_spec, ss, es,
                       alpha = alpha,
                       alternative = alternative)
      }, numeric(1))
      df
    ...
    output$sensitivity_table <- DT::renderDT({
      df <- sensitivity_table_with_power()
      shiny::req(df)
...
.render_generic_report_binary <- function(report_data, test_spec, fmt) {
  if (fmt == "pdf") {
    return(.render_via_rmarkdown(report_data, test_spec, "pdf"))
  }
  if (fmt == "word") {
