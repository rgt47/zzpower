...
The `zzpower` package provides a Shiny application for conducting power
analysis and sample size calculations for two-group parallel clinical
trial designs.

## Features

- **Multiple Effect Size Methods**: Support for Cohen’s d, percentage
  reductions, difference in change scores, and treatment group changes
- **Design Considerations**: Accounts for dropout rates, drop-in rates,
  and unequal group allocation ratios  
...
The development version may be installed of zzpower from
[GitHub](https://github.com/) with:

``` r

# install.packages('pak')
pak::pak('rgt47/zzpower')
```
...
The application will open in the default web browser and provide an
interactive interface for:

1.  **Sample Size Planning**: Set total sample size and expected dropout
    rates
2.  **Effect Size Specification**: Choose from multiple methods to
    specify the expected treatment effect
3.  **Advanced Configuration**: Adjust randomization ratios, Type I
    error rates, and testing approaches
4.  **Results Visualization**: View interactive power curves and
...
- **Standard Deviation Units (Cohen’s d)**: Direct specification of
  standardized effect sizes
- **Percent Reduction**: Effect size specified as percentage reduction
  from placebo  
- **Difference in Change Scores**: Absolute difference between treatment
  groups in outcome units
- **Change in Active Group**: Specifying the treatment group change
  directly
...
6.  Review the power curve and identify the detectable effect size at
    80% power
...
- [`vignette("quickstart")`](https://rgt47.github.io/zzpower/articles/quickstart.md) -
  Quick start guide
